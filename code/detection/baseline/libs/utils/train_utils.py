...
def valid_one_epoch(
    ...
    if evaluator is not None:
        ...
        # json_output_path = output_file.replace(".pkl", ".json")
        # save_results_as_json(results, json_output_path)
...
def save_results_as_json(results, output_json_path):
    output_dict = {
        "version": "LFAAC",
        "database": {}
    }
    ...
    for vid, t_start, t_end, label in zip(video_ids, starts, ends, labels):
        if vid not in output_dict['database']:
            output_dict['database'][vid] = {"annotations": []}
        output_dict['database'][vid]['annotations'].append({
            "label": str(label),
            "label_id": int(label),
