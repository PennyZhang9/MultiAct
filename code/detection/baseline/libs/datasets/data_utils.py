...
def truncate_feats(
    ...
    """
    Truncate feats and time stamps in a dict item 截断
    ...
    """
    # get the meta info
    feat_len = data_dict['feats'].shape[1] # 3719
    num_segs = data_dict['segments'].shape[0] # 89
    ...
    for _ in range(max_num_trials): # max_num_trials = 200

        # sample a random truncation of the video feats
        st = random.randint(0, feat_len - max_seq_len) # 随机起点
        ed = st + max_seq_len
        window = torch.as_tensor([st, ed], dtype=torch.float32) #  随机一个起始位置 [st, et], 也就是一个window

        # compute the intersection between the sampled window and all segments
        window = window[None].repeat(num_segs, 1)
        left = torch.maximum(window[:, 0] - offset, data_dict['segments'][:, 0]) # offset: 2.6
        right = torch.minimum(window[:, 1] + offset, data_dict['segments'][:, 1])
        inter = (right - left).clamp(min=0) # 计算窗口与每个segment的交集长度
        area_segs = torch.abs(
