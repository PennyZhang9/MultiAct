...
def perform_test(test_loader, model, test_meter, cfg):
    ...
    for cur_iter, (inputs, labels, video_idx, meta) in enumerate(test_loader):
        ...
        slowfast_feats, outputs, preds = model(inputs) 
        
        # cls
        slow_emb, fast_emb = outputs
        ann_id = meta['annotation_id'][0]
        record = {
            'slow': slow_emb.detach().cpu().numpy(),   # shape (1,2048,25,4)
            'fast': fast_emb.detach().cpu().numpy(),   # shape (1,256,100,4)
            'meta': meta                                # 保留全部 meta，日后追溯方便
        }
        all_records[ann_id] = record

        # det
        slowfast_feats_array = slowfast_feats.squeeze().cpu().numpy().reshape(1, -1)
        all_feats.append(slowfast_feats_array)
    ...
    save_det = cfg.get('SAVE_DETECTION', False)
    if save_det:
        slowfast_feats_array_total = np.concatenate(all_feats, axis=0) #[6459, 2304]
        np.savez_compressed(cfg.DETECTION_FEATS_PATH, feats=slowfast_feats_array_total.astype(np.float32)) 

    save_cls = cfg.get('SAVE_CLASSIFICATION', False)
    if save_cls:
        with open(cfg.CLS_FEATS_PATH, 'wb') as emb_f:
            pickle.dump(all_records, emb_f)
